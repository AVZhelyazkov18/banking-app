...
class PaymentPlanServiceImplTest {
    ...
    @Test
    void generateAnnuityPaymentPlanShouldCreateEqualInstallmentsWithDecreasingInterestAndIncreasingPrincipal() {
        LocalDate firstPaymentDate = LocalDate.of(2026, 7, 1);

        List<PaymentPlanDTO> result = paymentPlanService.generateAnnuityPaymentPlan(
        ...
        PaymentPlanDTO first = result.get(0);
        PaymentPlanDTO second = result.get(1);
        PaymentPlanDTO last = result.get(17);
        ...
        assertEquals(firstPaymentDate, first.getDate());

        assertTrue(second.getInterestPortion().compareTo(first.getInterestPortion()) < 0);
        assertTrue(second.getPrincipalPortion().compareTo(first.getPrincipalPortion()) > 0);

        assertEquals(firstPaymentDate.plusMonths(17), last.getDate());
    ...
    @Test
    void generateAnnuityPaymentPlanShouldWorkWithZeroInterest() {
        LocalDate firstPaymentDate = LocalDate.of(2026, 7, 1);
        ...
        for (PaymentPlanDTO installment : result) {
            assertEquals(new BigDecimal("100.00"), installment.getContributionAmount());
            assertEquals(new BigDecimal("100.00"), installment.getPrincipalPortion());
            assertEquals(new BigDecimal("0.00"), installment.getInterestPortion());
        }
        ...
        assertEquals(new BigDecimal("1200.00"), totalPrincipal);
    }

    @Test
    void generateAnnuityPaymentPlanShouldThrowWhenAmountIsNull() {
        IllegalArgumentException exception = assertThrows(
                IllegalArgumentException.class,
                () -> paymentPlanService.generateAnnuityPaymentPlan(null, 6.25, 18, LocalDate.now())
        );
    ...
    @Test
    void generateAnnuityPaymentPlanShouldThrowWhenAmountIsZeroOrNegative() {
        IllegalArgumentException exception = assertThrows(
                IllegalArgumentException.class,
                () -> paymentPlanService.generateAnnuityPaymentPlan(BigDecimal.ZERO, 6.25, 18, LocalDate.now())
        );
    ...
    @Test
    void generateAnnuityPaymentPlanShouldThrowWhenInterestIsNegative() {
        IllegalArgumentException exception = assertThrows(
                IllegalArgumentException.class,
                () -> paymentPlanService.generateAnnuityPaymentPlan(new BigDecimal("1000.00"), -1, 12, LocalDate.now())
        );

        assertEquals("Annual interest rate cannot be negative", exception.getMessage());
    }

    @Test
    void generateAnnuityPaymentPlanShouldThrowWhenTermIsInvalid() {
        IllegalArgumentException exception = assertThrows(
                IllegalArgumentException.class,
                () -> paymentPlanService.generateAnnuityPaymentPlan(new BigDecimal("1000.00"), 5, 0, LocalDate.now())
        );

        assertEquals("Payment term must be positive", exception.getMessage());
    }

    @Test
    void generateAnnuityPaymentPlanShouldThrowWhenFirstPaymentDateIsNull() {
        IllegalArgumentException exception = assertThrows(
                IllegalArgumentException.class,
                () -> paymentPlanService.generateAnnuityPaymentPlan(new BigDecimal("1000.00"), 5, 12, null)
        );

        assertEquals("First payment date is required", exception.getMessage());
    }
